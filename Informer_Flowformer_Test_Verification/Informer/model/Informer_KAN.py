...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.kan=KAN(width=[5,3,5], grid=4, k=3) 
    ...
    def forward(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None, mask=None):
        ...
        # 取最后时刻特征 → 映射到输出通道 [B, c_out]
        out_linear = self.projection(enc_out[:, -1, :])

        # KAN 残差/后置映射（保证输入维度与 c_out 一致）
        device = out_linear.device
        self.kan = self.kan.to(device)
        pred = self.kan(out_linear)              # [B, c_out]

        # 若下游训练/评估期望 [B, 1, c_out]，再扩一维
